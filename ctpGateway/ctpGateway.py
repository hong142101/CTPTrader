...
class CtpGateway(VtGateway):
    ...
    def __init__(self, eventEngine):
        """Constructor"""
        super(CtpGateway, self).__init__(eventEngine, "CTP")
    ...
    def connect(self):
        ...
        # 初始化并启动查询
        self.initQuery()
        while self.tdApi.ctp_get_all_contract_info is False:
            print("getting ctp all active contract info......")
            t.sleep(1)
...
class CtpMdApi(MdApi):
    ...
    def onRtnDepthMarketData(self, data):
        """行情推送"""
        if ((data['UpdateTime'] > '02:30:00') and (data['UpdateTime'] < '09:00:00')) or \
                ((data['UpdateTime'] > '15:00:00') and (data['UpdateTime'] < '21:00:00')):
            return
        ...
        # CTP只有一档行情
        if (data['BidPrice1'] > 1.0e+308) or (data['AskPrice1'] > 1.0e+308):
            tick.bidPrice1 = data['LastPrice']
            tick.askPrice1 = data['LastPrice']
        else:
...
class CtpTdApi(TdApi):
    ...
    def __init__(self, gateway):
        ...
        self.posDict = dict()  # 缓存持仓数据的字典
        self.symbolExchangeDict = dict()  # 保存合约代码和交易所的印射关系
        self.symbolSizeDict = dict()  # 保存合约代码和合约大小的印射关系

        self.requireAuthentication = False
        self.ctp_get_all_contract_info = False
    ...
    def onRspSettlementInfoConfirm(self, data, error, n, last):
        ...
        # 查询合约代码
        self.reqID += 1
        self.reqQryInstrument(dict(), self.reqID)
    ...
    def onRspQryInstrument(self, data, error, n, last):
        ...
        # 合约查询回报结束
        self.ctp_get_all_contract_info = True
    ...
    def onRtnTrade(self, data):
        ...
        datetime = dt.datetime.strptime(' '.join([data['TradeDate'], data['TradeTime']]), "%Y%m%d %H:%M:%S")
        datetime = datetime.strftime("%Y-%m-%d %H:%M:%S")
        trade.tradeTime = datetime
    ...
    def qryAccount(self):
        """查询账户"""
        self.reqID += 1
        self.reqQryTradingAccount(dict(), self.reqID)
