...
class CtaTemplate(object):
    ...
    long_pos_open_datetime = dt.datetime.max  # 最新仓位更新时间
    short_pos_open_datetime = dt.datetime.max  # 最新仓位更新时间
    pos = 0
    long_pos = 0
    short_pos = 0
    ...
    def loadTick(self):
        """读取tick数据"""
        return self.ctaEngine.loadTick(self.vtSymbol)

    # ----------------------------------------------------------------------
    def loadBar(self):
        """读取bar数据"""
        return self.ctaEngine.loadBar(self.vtSymbol)
    ...
    # ----------------------------------------------------------------------
    def updatePosition(self, strategy_name, trade):
        d = deepcopy(trade.__dict__)
        # 将键值加入字典
        d['name'] = strategy_name
        d['long_pos'] = self.long_pos
        d['short_pos'] = self.short_pos
        # 调用主引擎模块的数据库操作
        self.ctaEngine.updatePosition(strategy_name, d)
