...
strategy_config_path = os.path.join(root_path, "ctaStrategy", "strategy")
...
class CtaEngine(object):
    ...
    def processTradeEvent(self, event):
        ...
        if trade.vtOrderID in self.orderStrategyDict:
            ...
            if (trade.direction == DIRECTION_LONG) and (trade.offset == OFFSET_OPEN):
                strategy.long_pos += trade.volume
            elif (trade.direction == DIRECTION_SHORT) and (trade.offset == OFFSET_OPEN):
                strategy.short_pos -= trade.volume
            elif (trade.direction == DIRECTION_SHORT) and (
                (trade.offset == OFFSET_CLOSE) or (trade.offset == OFFSET_CLOSETODAY) or (
                    trade.offset == OFFSET_CLOSEYESTERDAY)):
                strategy.long_pos -= trade.volume
            elif (trade.direction == DIRECTION_LONG) and (
                (trade.offset == OFFSET_CLOSE) or (trade.offset == OFFSET_CLOSETODAY) or (
                    trade.offset == OFFSET_CLOSEYESTERDAY)):
                strategy.short_pos += trade.volume
    ...
    def insertData(self, dbName, collectionName, data):
        ...
        self.mainEngine.mysqlInsert(dbName, collectionName, data.__dict__)

    # ----------------------------------------------------------------------
    def loadBar(self, instrument):
        """从数据库中读取Bar数据，startDate是datetime对象"""
        history_data_file = os.path.join(history_data_path, instrument + ".csv")

        barData = list(csv.DictReader(open(history_data_file, 'r')))
        barData.sort(key=lambda x: x['datetime'])

        for a in barData:
            a['datetime'] = datetime.strptime(a['datetime'], "%Y-%m-%d %H:%M:%S")
            a['openInterest'] = int(a['openInterest'])
            a['open'] = float(a['open'])
            a['high'] = float(a['high'])
            a['low'] = float(a['low'])
            a['close'] = float(a['close'])
            a['volume'] = int(a['volume'])
    ...
    # ----------------------------------------------------------------------
    def updatePosition(self, strategy_name, data):
        """插入数据到数据库（这里的data可以是CtaTickData或者CtaBarData）"""
        strategy_config_file = os.path.join(strategy_config_path, strategy_name + ".stg")
        self.mainEngine.update_strategy_config(strategy_config_file, data)

    # ----------------------------------------------------------------------
    def loadPosition(self):
        """从数据库载入策略的持仓情况"""
        for strategy in list(self.strategyDict.values()):
            strategy_config_file = os.path.join(strategy_config_path, strategy.name + ".stg")
            strategy_positions = self.mainEngine.parse_strategy_config(strategy_config_file)
            strategy.long_pos = strategy_positions['long']['quantity']
            strategy.long_pos_open_datetime = strategy_positions['long']['datetime']
            strategy.short_pos = strategy_positions['short']['quantity']
            strategy.short_pos_open_datetime = strategy_positions['short']['datetime']
